...
Work through these four stages. If the user has already answered some of these, skip ahead.

### Stage 1 — Gather context

Ask the following in a single message (not four separate ones):

- What type of product is this? (productivity, content/media, utility, marketplace, etc.)
...
Before evaluating any model on its merits, apply the structural market constraint. The market determines which models are viable before product fit or team capacity are even considered.
...
- **Payment infrastructure:** Does the chosen gateway support recurring billing for the target market and model? (Reference `payment-methods.md` for market-specific recurring support)
